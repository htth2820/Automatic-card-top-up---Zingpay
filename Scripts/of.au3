...
Func _Add_Strings($aList, $AU3, $splitChar)
    ...
    For $A = 0 To UBound($aList) - 1
        If $aList[$A] <> "" And StringInStr($aList[$A], "_CryptoDragon_Crypt") = False And StringLeft($aList[$A], 1) <> "'" And StringRight($aList[$A], 1) <> "'" And StringLeft($aList[$A], 1) <> '"' And StringRight($aList[$A], 1) <> '"' And StringLen($aList[$A]) > 1 Then
			If StringInStr($aList[$A], @CRLF) Then
			   $aList[$A] = StringReplace($aList[$A], @CRLF, "")
			EndIf
			Global $sPass = ""
			Dim $sRndChr[3]
			...
			Next
			If StringLen($aList[$A]) >= 2000 Then
			   ContinueLoop
			EndIf
...
Func _Write_CryptoDragon_Obfuscated_File($AU3) ; Write CryptoDragon Obfuscated File
    Local $file_Simple_Obfuscated = StringTrimRight($file_Script_to_Obfuscate, 4) & " - CryptoDragon Obfuscated.au3" ; Create CryptoDragon Obfuscated Filename
