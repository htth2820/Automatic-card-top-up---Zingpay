	...
	Switch $nMsg
		...
		Case $recharge
			If $game_name = 'gunny' Then
				If StringLen(FileReadLine($file,1)) <> StringLen(FileReadLine($file,2)) Then
					_Recgunny1()
				Else
					_Recgunny2()
				EndIf
...
#ce		$cardpass = StringReplace($acardpass[2], '-', '')
		If StringLen(FileReadLine($file,$i)) > StringLen(FileReadLine($file,$i +1) Then
			$serial = FileReadLine($file,$i)
			$cardpass = FileReadLine($file,$i+1)
		Else
			$serial = FileReadLine($file,$i+1)
			$cardpass = FileReadLine($file,$i)
		EndIf
		$t = ($i + 1) / 2
