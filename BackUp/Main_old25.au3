...
Dim $sname[411], $sc_serverID[411], $gn_sID[411]
For $i = 1 To $tach[0]
...
Func _Recgunny1()
	For $i = 1 To _FileCountLines($file) Step 2
		...
		If StringLen($serial) <> 12 Then
			...
			GUICtrlCreateListViewItem($serial & '|' & $cardpass & '|' & 'Failed', $ListView1)
		Else
			$rec = _HttpRequest(2, 'https://new.pay.zing.vn/ajax/payment-zingcard', 'pmcID=1&pmOptID=' & $pmOptID & '&sID=' & $gn_sID[$i] & '&roleID=' & $roleID & '&roleName=' & $roleName & '&productCode=' & $game_name & '&cardSerial=' & $serial & '&cardPassword=' & $cardpass & '&accountName=' & $acc, '', 'https://new.pay.zing.vn/payment/' & $game_name, 'X-Requested-With: XMLHttpRequest')
