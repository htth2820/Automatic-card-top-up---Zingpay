...
$Form1 = GUICreate("AutoZingPay ver 1.0.0 - by Tran Thanh Hung", 609, 448, 359, 157)
GUISetFont(10, 400, 0, "MS Sans Serif")
GUISetBkColor(0x646464, $Form1)

$ListView1 = GUICtrlCreateListView("Serial|CardCode|Status", 256, 128, 217, 305, BitOR($ES_AUTOVSCROLL, $ES_READONLY, $WS_VSCROLL))
...
$BLogin = GUICtrlCreateButton("LOGIN", 504, 8, 81, 33)
GUICtrlSetFont(-1, 10, 800, 0, "MS Sans Serif")
GUICtrlSetColor(-1, 0x00FF00)
GUICtrlSetBkColor(-1, 0x000000)
...
$Confirm = GUICtrlCreateButton("CONFIRM", 504, 56, 81, 33)
GUICtrlSetFont(-1, 10, 800, 0, "MS Sans Serif")
GUICtrlSetColor(-1, 0x00FF00)
GUICtrlSetBkColor(-1, 0x000000)
GUISetState(@SW_SHOW)
...
#ce	$accountID = StringRegExp($login, 'var accountID="(.*?)";', 1)
	If @error Then
	...
	Else
		...
		#cs		$load_payment = _HttpRequest(1, 'https://pay.zing.vn/ajax/check-account?productCode=tanthienlong3d&accountName=' & $acc & '&accountID=' & $accountID[0] & '&accountGuid=' & $accountGuid[0] & '&serverID=' & $serverID[0] & '&roleID=', '', '', 'https://pay.zing.vn/product/tanthienlong3d?mess=succ&u=' & $acc & '', 'X-Requested-With: XMLHttpRequest')
		...
		#ce		$load_payment = _HttpRequest(1, 'https://pay.zing.vn/ajax/check-account?productCode=tanthienlong3d&accountName=' & $acc & '&accountID=' & $accountID[0] & '&accountGuid=' & $accountGuid[0] & '&serverID=' & $serverID[0] & '&roleID=', '', '', 'https://pay.zing.vn/product/tanthienlong3d?mess=succ&u=' & $acc & '', 'X-Requested-With: XMLHttpRequest')
...
#ce			ShellExecute(@ScriptDir & '\test.html')
...
Func _Rec2()
	For $i = 1 To _FileCountLines($file) Step 1
		...
		If StringLen($serial) <> 12 Then
			...
			GUICtrlCreateListViewItem($serial & '|' & $cardpass & '|' & 'Failed', $ListView1)
		Else
			$rec = _HttpRequest(2, 'https://pay.zing.vn/ajax/payment-zingcard', 'pmcID=1&pmOptID=35&sID=' & $sID[0] & '&roleID=&roleName=&productCode=' & $game_name & '&cardSerial=' & $serial & '&cardPassword=' & $cardpass & '&accountName=' & $acc & '', '', 'https://pay.zing.vn/payment/' & $game_name, 'X-Requested-With: XMLHttpRequest')
			$aID = StringRegExp($rec, '"transID":(.*?)\}', 1)
			$link_cf = 'https://new.pay.zing.vn/result/' & $game_name & '/' & $aID[0]
			$cf = _HttpRequest(2, $link_cf, 'pmcID=1&pmOptID=35', '', 'https://new.pay.zing.vn/payment/' & $game_name, 'Upgrade-Insecure-Requests: 1')
			$result = _HttpRequest(2, 'https://new.pay.zing.vn/ajax/get-result', 'transID=' & $aID[0], '', $link_cf, 'X-Requested-With: XMLHttpRequest')
			If StringInStr($result, 'Thẻ đã được sử dụng') > 0 Then
...
;==============================_Volam========================
Func _Volam()
	$volam = _HttpRequest(2, 'https://new.pay.zing.vn/product/volamtruyenky', '', '', 'https://new.pay.zing.vn/product/volamtruyenky', 'Upgrade-Insecure-Requests: 1')
	FileDelete(@ScriptDir & '\test.html')
