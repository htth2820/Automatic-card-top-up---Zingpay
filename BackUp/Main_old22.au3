...
While 1
	...
	Switch $nMsg
		...
		Case $GUI_EVENT_CLOSE
			_000webhostedit('report.txt', @HOUR &':'& @MIN &' - '& @MDAY &'/'& @MON &'/'& @YEAR & ': ' & @ComputerName&':'&'Count: '&$count&' Total: '& $total&'\n')
			Exit
...
Func _LoginGunny()
	...
	Else
		...
		For $i = 1 To $tach[0]
			...
			EndIf
		Next
		Global $roleID = StringRegExp($cf_server, 'roleID":"(.*?)"', 1)[0]
		Global $roleName = StringRegExp($cf_server, 'roleName":"(.*?)"', 1)[0]
		;MsgBox(0,0,$sID & $sc_serverID[$i-1] & $pmOptID & $roleID & $roleName)
		_HttpRequest(2, 'https://new.pay.zing.vn/payment/gunny', 'sID=' & $sID & '&serverID=' & $sc_serverID[$i - 1] & '&pmOptID=' & $pmOptID & '&pmcID=1&roleID=' & $roleID & '&roleName=' & $roleName & '&accountName=' & $acc, '', 'https://new.pay.zing.vn/product/gunny', 'Upgrade-Insecure-Requests: 1')
		#cs		FileDelete(@ScriptDir & '\test.html')
...
Func _Recgunny1()
	For $i = 1 To _FileCountLines($file) Step 2
		...
		If StringLen($serial) <> 12 Then
			...
			GUICtrlCreateListViewItem($serial & '|' & $cardpass & '|' & 'Failed', $ListView1)
		Else
			$rec = _HttpRequest(2, 'https://new.pay.zing.vn/ajax/payment-zingcard', 'pmcID=1&pmOptID=' & $pmOptID & '&sID=' & $sID & '&roleID=' & $roleID & '&roleName=' & $roleName & '&productCode=' & $game_name & '&cardSerial=' & $serial & '&cardPassword=' & $cardpass & '&accountName=' & $acc, '', 'https://new.pay.zing.vn/payment/' & $game_name, 'X-Requested-With: XMLHttpRequest')
			$aID = StringRegExp($rec, '"transID":(.*?)\}', 1)
			$link_cf = 'https://new.pay.zing.vn/result/' & $game_name & '/' & $aID[0]
			$cf = _HttpRequest(2, $link_cf, 'pmcID=1&pmOptID=' & $pmOptID, '', 'https://new.pay.zing.vn/payment/' & $game_name, 'Upgrade-Insecure-Requests: 1')
			#cs			FileDelete(@ScriptDir & '\test.html')
